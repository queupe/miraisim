    ...
    for param in range(PARAMS):
        ...
        for rnd in range(ROUNDS):
            ...
            if PRINT_ROUND:
                ...
                web_temp_name ='web/exec-{:>03d}_param-{:>02d}_temp.html'.format(counter,param)
                with open(web_temp_name, 'a') as web_temp_file:
                    web_temp_file.write("<BR>")
                    web_temp_file.write("Parameters[{}]:{}: {} \t Round:{:>2d}\n".format(param+1,str_param, elem_param,rnd+1))
                    web_temp_file.write("<table>\n")
                    web_temp_file.write("\t<tr>\n")
                    web_temp_file.write("\t\t<th> </th> <th>min</th> <th>max</th> <th>average</th> <th>proportion</th>\n")
                    web_temp_file.write("\t</tr>\n\t<tr>\n")
                    web_temp_file.write("\t\t<td> heal  </td> <td> {}  </td> <td> {}  </td> <td> {:8.4f} </td> <td>{:>1.4f}</td>\n".format (h_min, h_max, avg_heal,  avgt_heal))
                    web_temp_file.write("\t</tr>\n\t<tr>\n")
                    web_temp_file.write("\t\t<td> infec </td> <td> {}  </td> <td> {}  </td> <td> {:8.4f} </td> <td>{:>1.4f}</td>\n".format (i_min, i_max, avg_infec, avgt_infec))
                    web_temp_file.write("\t</tr>\n\t<tr>\n")
                    web_temp_file.write("\t\t<td> off   </td> <td> {}  </td> <td> {}  </td> <td> {:8.4f} </td> <td>{:>1.4f}</td>\n".format (o_min, o_max, avg_off,   avgt_off))
                    web_temp_file.write("\t</tr>\n\t<tr>\n")
                    web_temp_file.write("\t\t<td>All    </td> <td>  -  </td> <td> -   </td> <td> {:8.4f} </td> <td>{:>1.4f}</td>.................\n".format (avg_all, avgt_all))
                    web_temp_file.write("\t</tr>\n")
                    web_temp_file.write("</table>\n")
                    web_temp_file.write("<table>\n")
                    ...
                    web_temp_file.write("\t</tr>\n\t<tr>\n")
                    web_temp_file.write("\t\t<td> Tx attempt to infect end      </td> <td> {:.5e}</td>\n".format(float(sim.attempt_infected_endogeno)/(sim.config["maxhid"]*time_befo)))
                    web_temp_file.write("\t</tr>\n\t<tr>\n")
                    web_temp_file.write("\t\t<td> Tx attempt to infect exo      </td> <td> {:.5e}</td>\n".format(float(sim.attempt_infected_exogeno)/time_befo))
                    web_temp_file.write("\t</tr>\n")
                    web_temp_file.write("</table>\n")
        ...
        if WEB_PARAM:
            ...
            plt.ylim((0.0,1.5))
            plt.legend(loc='upper right')
            fig_name ='img/fig_exec-{:>03d}_param-{:>02d}'.format(counter,param)
            plt.savefig('web/'+ fig_name + '.png', dpi=my_dpi)
            #plt.show()
            plt.clf()
            ...
            fig0_name ='img/fig_exec-{:>03d}_param-00.png'.format(counter)
            fig1_name ='img/fig_exec-{:>03d}_param-01.png'.format(counter)
            fig2_name ='img/fig_exec-{:>03d}_param-02.png'.format(counter)
            fig3_name ='img/fig_exec-{:>03d}_param-03.png'.format(counter)
            fig4_name ='img/fig_exec-{:>03d}_param-04.png'.format(counter)
            # File for this Figure
            # head
            head_html_name = 'web/head.html'
            html_name ='web/exec-{:>03d}_param-{:>02d}.html'.format(counter,param)
            with open(head_html_name) as f:
                with open(html_name, "w") as f1:
                    for line in f:
                        f1.write(line)

            # Body with graphs
            with open(html_name,"a") as the_html:
                the_html.write("      <figure><img src=\"{}.png\" alt=\"{}\">\n".format(fig_name,str_param))
                ...
                if param == 1:
                    ...
                    the_html.write("                <li>Average time between infection(bot):</li>\n")
                ...
                if param == 2:
                    ...
                    the_html.write("                <li>Average time between infection(master):</li>\n")
                    the_html.write("                <li>{:.5e}</li>\n".format(sim.config['bot']['master'][0]))
                ...
                if param != 1:
                    ...
                    the_html.write("                               <a href=\"{}\">Time between endogenous infection</a>\n".format(html1_name))
                    the_html.write("              </li>\n")

                if param != 2:
                    the_html.write("              <li>             <a href=\"{}\"><img src=\"{}\" width=\"220\" height=\"165\" alt=\"\"></a>\n".format(html2_name, fig2_name))
                    the_html.write("                               <a href=\"{}\">Time between exogenous infection</a>\n".format(html2_name))
                    the_html.write("              </li>\n")
            ...
            # Detail of execution
            with open(web_temp_name) as f:
                with open(html_name, "a") as f1:
                    for line in f:
                        f1.write(line)

            # Foot HTML
            foot_html_name = 'web/foot.html'
            with open(foot_html_name) as f:
                with open(html_name, "a") as f1:
                    for line in f:
                        f1.write(line)

...
title('exact value and approximated with 3 iterations');
...
for gamma1 = gamma1_vec
    igamma = igamma +1;
    for n = N
        rho0(igamma, n) = rho_zero_3iter(lambda1/n, mu1, gamma1, n-1);
        rho1(igamma, n) = rho_one_3iter (lambda1/n, mu1, gamma1, n-1);
    end

    for n = N
        txt = sprintf('gamma=%f;\t N=%d;\t rho0(0)=%f;\t rho0(1)=%f ', gamma1, n, rho0(igamma, n), rho1(igamma, n));
        disp(txt);
    end
...
str_file_pdf = sprintf('exact_approx_lambda%0.2f_gamma%0.2f_iter_%d', lambda1, gamma1, 3);

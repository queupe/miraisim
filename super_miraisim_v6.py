    ...
    for param in range(PARAMS):
        ...
        for rnd in range(ROUNDS):
            ...
            if PLOT_ROUND or WEB_ROUND:
                ...
                # Suscetible
                plt.plot(hist_array[:,3], hist_array[:,0]/(sim.config['maxhid'] +1), label='susceptible'   , color='b')
                # Infected (without origin)
                plt.plot(hist_array[:,3], (hist_array[:,7]+hist_array[:,8])/(sim.config['maxhid'] +1), label='infected'      , color='r')
                # Endogenous infected
                plt.plot(hist_array[:,3], hist_array[:,7]/(sim.config['maxhid'] +1), label='endo infected' , color='#ee0000', linestyle = ':')
                # Exogenous infected
                plt.plot(hist_array[:,3], hist_array[:,8]/(sim.config['maxhid'] +1), label='exo infected'  , color='#ee0055', linestyle = '--')
                # OFF
                plt.plot(hist_array[:,3], hist_array[:,2]/(sim.config['maxhid'] +1), label='off'           , color='g')
                # Total
                ...
                fig_round_name ='img/fig_exec-{:>03d}_param-{:>02d}_round-{:>02}.png'.format(counter,param,rnd)
            ...
            if PRINT_ROUND:
                ...
                # file output
                filename ='mirai_sim_coun{:>03d}_param{:>03d}_{}.txt'.format(counter,param,str_param)
                if rnd == 0:
        ...
        if PLOT_PARAM:
            ...
            plt.ylim((0.0,1.7))
            plt.legend(loc='upper right')
            filename_pdf ='exec_counter-{:>03d}_param-{:>03d}_{}.pdf'.format(counter,param,str_param)
        ...
        if WEB_PARAM:
            ...
            plt.ylim((0.0,1.7))
            plt.legend(loc='upper right')
            fig_name ='img/fig_exec-{:>03d}_param-{:>02d}.png'.format(counter,param)
